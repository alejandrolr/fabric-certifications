...
var invokeChaincode = async function (peerNames, channelName, chaincodeName, fcn, args, username, org_name) {
	...
	try {
		...
		for (var i in proposalResponses) {
			...
			} else if (proposalResponses && proposalResponses[i].status === 500) {
				chaincodeError = true;
				error_message = 'chaincode Error: ' + proposalResponses[i].message;
			}
	...
	if (!error_message) {
		...
		let msg = {
			...
			peerMessage: proposalResponses[0].response.payload.toString()
		};
		logger.info(msg);
