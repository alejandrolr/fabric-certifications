...
var getRegisteredNewUser = async function (username, userOrg, attrs, isJson) {
	try {
...
var getRegisteredUser = async function (username, userOrg, isJson) {
	try {
		...
		if (user && user.isEnrolled()) {
			...
			// user was not enrolled, so we will need an admin user object to register
			logger.warn('User %s was not enrolled, it is needed an admin user object to register', username);
			// var admins = hfc.getConfigSetting('admins');
			// let adminUserObj = await client.setUserContext({ username: admins[0].username, password: admins[0].secret });
			// let caClient = client.getCertificateAuthority();
			// let secret = await caClient.register({
		...
		if (user && user.isEnrolled) {
			...
			throw new Error('User ' + username + ' was not enrolled, it is needed to register it before the enrollment process');
		}
	} catch (error) {
...
exports.getRegisteredNewUser = getRegisteredNewUser;
