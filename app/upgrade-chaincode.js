...
var upgradeChaincode = async function (peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
	...
	try {
		...
		var request = {
			...
			'endorsement-policy': {
				...
				policy: {
					...
					// EDITED: only one signature from one only member
		...
		let results = await channel.sendUpgradeProposal(request, 60000); //upgrade takes much longer
	...
	if (!error_message) {
		let message = util.format(
			'Successfully upgrade chaincode to version %s in organization %s to the channel \'%s\'',
			chaincodeVersion, org_name, channelName);
		logger.info(message);
