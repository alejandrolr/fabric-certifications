...
app.use(expressJWT({
	...
	path: ['/enrollUser', '/newUser']
}));
app.use(bearerToken());
app.use(function (req, res, next) {
	logger.debug(' ------>>>>>> new request for %s', req.originalUrl);
	if (req.originalUrl.indexOf('/enrollUser') >= 0 || req.originalUrl.indexOf('/newUser') >= 0) {
		return next();
	}
		...
		if (err) {
			res.send({
				...
				message: 'Failed to authenticate token. Make sure to include the ' +
					'token returned from /enrollUser or /newUser call in the authorization header ' +
					' as a Bearer token'
			});
			return;
		} else {
...
///////////////////////////////////////////////////////////////////////////////
// Register and enroll new user with a role (only admin)
app.post('/newUser', async function (req, res) {
	var username = req.body.username;
	var orgName = req.body.orgName;
	var attrs = req.body.attrs;
	logger.debug('End point : /NewUser');
	logger.debug('User name : ' + username);
	logger.debug('Org name  : ' + orgName);
	logger.debug('Attrs  : ' + JSON.stringify(attrs));
	if (!username) {
	...
	if (!attrs) {
		res.json(getErrorMessage('\'attrs\''));
		return;
	}
	var token = jwt.sign({
	...
	let response = await helper.getRegisteredNewUser(username, orgName, attrs, true);
	logger.debug('-- returned from registering the username %s for organization %s with attrs %s', username, orgName, JSON.stringify(attrs));
	if (response && typeof response !== 'string') {
		logger.debug('Successfully registered the username %s for organization %s with attrs %s', username, orgName, JSON.stringify(attrs));
		response.token = token;
		res.json(response);
	} else {
		logger.debug('Failed to register the username %s for organization %s with::%s', username, orgName, JSON.stringify(response));
...
// Enroll user
app.post('/enrollUser', async function (req, res) {
	var username = req.body.username;
	var orgName = req.body.orgName;
	logger.debug('End point : /enrollUser');
	logger.debug('User name : ' + username);
	logger.debug('Org name  : ' + orgName);
	if (!username) {
		res.json(getErrorMessage('\'username\''));
	...
	if (response && typeof response !== 'string') {
		...
		logger.debug('Failed to enroll the username %s for organization %s with::%s', username, orgName, response);
		res.json({ success: false, message: response });
	}
...
app.post('/channels/:channelName/upgradeChaincodes', async function (req, res) {
